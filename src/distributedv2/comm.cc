...
: buffer(length) {
  std::memset(buffer.data(), 0, buffer.size() * sizeof(iov_pool_item_t));
}
...
int IovPool::alloc(iov_pool_item_t** item) {
  ...
  if (p->used) return 1;
  p->used = true;
  for (void *data: p->data) {
    free(data);
  }
  p->data.resize(0);
  std::memset(p->iov, 0, MAX_IOV_CNT * sizeof(comm_iov_t));
  p->iov_cnt = 0;
  *item = p;
...
ucs_status_t Communicator::recv_cb(
  ...
  LOG(INFO) << "recv_cb: iov_cnt=" << iov_cnt;
...
void Communicator::send(int rank, unsigned id, comm_chunk_t *chunk) {
  ucp_request_param_t params = {
    .op_attr_mask = UCP_OP_ATTR_FIELD_DATATYPE | UCP_OP_ATTR_FIELD_CALLBACK | UCP_OP_ATTR_FIELD_FLAGS,
...
unsigned Communicator::add_recv_handler(void *arg, comm_cb_handler_t cb) {
  ...
  chunks.assign(size, std::vector<comm_chunk_t>(id, { NULL, &pool}));
  return id;
}
...
void Communicator::progress() {
  ...
  for (int destrank = 0; destrank < size; destrank++) {
    ...
    for (unsigned id = 0; id < recv_handlers.size(); id++) {
      if (pool.empty(chunks[rank][id].item)) continue;
      send(destrank, id, &chunks[rank][id]);
    }
  }
}
