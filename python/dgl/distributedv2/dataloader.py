...
class RangePolicy:
    ...
    def load_temp_edge(self, rank: int, edge):
        self._rank = rank
...
"""
def create_distgraph(rank: int, size: int, num_nodes: int, edge):
    policy = RangePolicy(size, num_nodes)
    temp_edge = policy.load_temp_edge(rank, edge)
    vals, cnts = policy.count_dest_rank(temp_edge)

    # s_counts is how many edges this proc will send to others.
    s_counts = array('Q', [0] * size)
    for val, cnt in np.stack((vals, cnts), axis = 1):
        s_counts[val] = cnt

    comm = MPI.COMM_WORLD
    assert comm.Get_size() == size, "size is invalid"
    assert comm.Get_rank() == rank, "rank is invalid"

    # r_counts is how many edges this proc will fetch from others.
    r_counts = array('Q', [0] * size)
